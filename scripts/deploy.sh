...
if [ -z "$HF_SPACE_NAME" ]; then
    echo "${YELLOW}找不到 HF_SPACE_NAME 環境變數，使用預設名稱 'markdown-vault'${NC}"
    HF_SPACE_NAME="markdown-vault"
fi

# 檢查並安裝必要的 Python 套件
echo "${YELLOW}檢查必要的 Python 套件...${NC}"
uv run python -c "
import sys
required_packages = ['huggingface_hub', 'gradio']
missing_packages = []
...
if missing_packages:
    print(f'缺少套件: {missing_packages}')
    sys.exit(1)
else:
    print('所有必要套件已安裝')
" || {
    echo "${RED}缺少必要的 Python 套件，請先執行: pip install -e .${NC}"
    exit 1
}
...
try:
    ...
    api.upload_folder(
        ...
        repo_id=repo_id,
        repo_type='space',
        commit_message='Deploy Markdown Vault application'
    )
    print('文件上傳完成!')
